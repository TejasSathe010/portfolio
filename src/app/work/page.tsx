...
export default function WorkPage() {
  return (
    <div className="space-y-12 sm:space-y-16">
      ...
      <Reveal>
        <section id="roles" className="scroll-mt-28 py-16 md:py-24 space-y-6 anchor-target" aria-label="Roles">
          {work.map((w) => {
                ...
                <CardHeader>
                  <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                    ...
                    <div className="flex items-center gap-2 text-xs text-fg/80">
                      <span className="rounded-full border border-border bg-surface px-3 py-1 font-semibold text-fg/80 leading-5 chip">
                        {w.start} — {w.end}
                      </span>
                      <a
                        href={`#${id}`}
                          ...
                          <div>
                            ...
                            <div className="mt-3 flex flex-wrap gap-2">
                              {w.stack.slice(0, 14).map((s) => (
                                <span
                                  key={s}
                                  className="rounded-full border border-border bg-surface px-3 py-1 text-[11px] font-semibold text-fg/80 leading-5 chip"
                                >
                                  {s}
                                </span>
                              ))}
      ...
      <section className="scroll-mt-28 py-16 md:py-24 anchor-target" id="projects" aria-label="Projects">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {projects.map((p, idx) => {
            ...
            return (
              <Reveal key={p.name} delayMs={idx * 60}>
                  ...
                  <CardHeader>
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {p.timeframe && (
                            <span className="rounded-r-chip border border-secondary/30 bg-secondary-tint/8 px-2.5 py-1 text-[10px] font-medium text-fg/70 leading-4 whitespace-nowrap chip">
                              {p.timeframe}
                            </span>
                          )}
                        </div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {p.stack.slice(0, 8).map((s) => (
                        <span
                          key={s}
                          className="rounded-r-chip border border-border bg-surface px-2.5 py-1 text-[10px] font-medium text-fg/70 leading-4 chip"
                        >
                          {s}
                        </span>
                      ))}
                      {p.stack.length > 8 && (
                        <span className="rounded-r-chip border border-border bg-surface-2 px-2.5 py-1 text-[10px] font-medium text-fg/60 leading-4 chip">
                          +{p.stack.length - 8}
                        </span>
                      )}
                    </div>
      ...
      <Reveal>
        <section className="scroll-mt-28 py-16 md:py-24 anchor-target" id="education" aria-label="Education">
          ...
          <div className="grid gap-4 sm:gap-6 lg:gap-8 sm:grid-cols-2">
            {education.map((e, idx) => (
              <Reveal key={e.school} delayMs={idx * 60}>
                <Card>
                  <CardHeader>
